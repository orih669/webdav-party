...
String.prototype.parseDelimiter = function (delimiter, keys, map)
{
    var output = {};
...
String.prototype.extract = function (prefix, postfix)
{
    ...
    if (prefix)
        start_index = this.indexOf(prefix) + prefix.length;

    if (postfix) 
        end_index = this.indexOf(postfix);

    if (start_index > -1 && end_index > start_index)
        return this.substr(start_index, end_index - start_index);
